...
We assume a working installation of `codacore` ([link](https://github.com/egr95/R-codacore/blob/main/README.md)), as well as the `selbal` package ([link](https://github.com/UVic-omics/selbal/blob/master/vignettes/vignette.Rmd)), from which we will borrow the Crohn's disease data (Gevers et al. 2014 http://dx.doi.org/10.1016/j.chom.2014.02.005).
```{r}
library("codacore")
library("selbal")
data("Crohn")
...
We consider the HIV data from (Noguera-Julian et al. 2016  http://dx.doi.org/10.1016/j.ebiom.2016.01.032), which again we take from the `selbal` package. The goal here is to construct a log-ratio of the microbial abundances that is predictive of the inflammation marker "sCD14", a continuous response variable. CoDaCoRe can be applied much in the same way, except the loss function changes from binary cross-entropy to mean-squared-error. This change will happen automatically based on the values received from `y` (although it can also be overriden manually via the ```objective``` parameter, for example, if the user wanted to fit a binary response using the mean-squared-error, they could specify ```objective = 'regression'```).
